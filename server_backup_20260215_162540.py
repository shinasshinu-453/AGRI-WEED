...
def get_crop_type(label: str, crop_name: str) -> str:
    ...
    weed_keywords = [
        # Generic weed terms
        'weed', 'wild', 'unwanted',
        # Weeds from your dataset (data.yaml)
        'waterhemp', 'morningglory', 'ragweed', 'cocklebur', 'spurred_anteria', 'spurred anteria',
        'prickly_sida', 'prickly sida', 'velvetleaf', 'palmer_amaranth', 'palmer amaranth',
        'redroot_pigweed', 'redroot pigweed', 'johnsongrass', 'tall_morningglory', 'tall morningglory',
        'sicklepod',
        # Common broadleaf weeds
        'pigweed', 'amaranth', 'lambsquarters', 'lamb\'squarters',
        'smartweed', 'dock', 'plantain', 'chickweed', 'purslane', 'spurge',
        # Thistle family
        'dandelion', 'thistle', 'canada thistle', 'bull thistle',
        # Vines and climbers
        'bindweed', 'morning-glory', 'morningglories', 'field bindweed',
        # Grassy weeds
    ...
    # Priority 4: Generic plant labels - assume weed if no explicit crop context
    if any(plant_term in label_lower for plant_term in plant_keywords):
        ...
        if crop_name_lower and crop_name_lower != 'general crop':
            ...
            # No crop context, could be either - default to weed
            print(f"   ? Classified as WEED (generic plant, no specific context - defaulting to weed)")
            return 'weed'
...
def detect_with_roboflow(image_data: bytes, crop_name: str = "Wheat") -> dict:
    ...
    try:
        ...
        params = {
            "api_key": ROBOFLOW_API_KEY,
            "confidence": 5,  # 5% confidence threshold - very low to catch all possible weeds
            "overlap": 30
        }
        
        print(f"📡 Sending to Roboflow: {url}")
        print(f"📋 Request params: confidence={params['confidence']}%, overlap={params['overlap']}%")
        ...
        if response.status_code != 200:
            print(f"❌ Roboflow API returned status {response.status_code}")
        ...
        print(f"📦 Full Roboflow response: {json.dumps(result, indent=2)[:1000]}")
        ...
        for pred in predictions:
            ...
            # Roboflow returns confidence as 0-1 already, NOT 0-100
            confidence = pred.get('confidence', 0)
            print(f"🔎 Raw prediction: class={class_name}, conf={confidence}, pred={pred}")
...
def analyze():
    ...
    try:
        ...
        print(f"\n📋 Request: Running BOTH models simultaneously")
        ...
        for result in results:
            if result.boxes is not None:
                print(f"🔍 YOLO found {len(result.boxes)} detections")
        ...
        print(f"✅ YOLO: {yolo_weed_count} weeds, {yolo_crop_count} crops in {yolo_time:.3f}s")
        ...
        if 'error' not in roboflow_result:
            ...
            print(f"✅ Roboflow: {roboflow_weed_count} weeds, {roboflow_crop_count} crops in {roboflow_time:.3f}s")
        else:
        ...
        orig_h, orig_w = img.shape[:2]
        img = resize_image_for_inference(img, max_size=640)  # Increased from 320 to preserve detail
        h, w = img.shape[:2]

        start_infer = time.time()
        results = model(img, conf=0.1, verbose=False, device=device, imgsz=640, max_det=100)  # Lower conf to 0.1 (10%)
